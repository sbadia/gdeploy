...
#def submit_api()
#	puts "--> Submit job on site #{SITE}"
#	Restfully::Session.new(:base_uri => config['url'] + '/2.0/grid5000', :username => USER, :password => config["password"]) do |root, session|
#		begin
#		env = (ENV['ENV_NAME'] == "prod") ? "#{ENV['ENV_NAME']}-#{site}" : "#{ENV['ENV_NAME']}"
#		myjob = root.sites[:"#{SITE}"].jobs.submit(:resources => "nodes=1,walltime=2", :command => "KAENV=#{ENV['KAENV']} RLS=#{ENV['RLS']} ENV_NAME=#{env} ruby ~/kaenvs/#{ENV['KAENV']}/gen_    oar.rb", :name => "Kaenv_generation", :types => ['deploy'] )
#		rescue
#			raise "Unable to submit job"
#		end
#		puts "--> Job #{myjob['uid']} with user #{myjob['user_uid']} submited ! (notification on notifications@conference.jabber.grid5000.fr)"
#	end
#end
